...
_.post("/product", upload.fields([{ name: "url" }]), newCourse); //completed//23//11
_.get("/product", getCourse); //completed//23//11
_.get("/product-id/:pid", getCourseById); //26/11
_.get("/product-cat/:pid", getCourseByCat); //26/11
_.delete("/product/:id", delCourse); //26/11
...
_.get("/customer", allStudent); // 29//11
_.get("/customer/:regi", searchStd); // 29//11
_.post("/admin", getAdmin); // 29//11
_.get("/admin", getAdminn); // 29//11
...
_.post("/order", myBooked);
_.post("/order/:id", updateOrder);
_.get("/order", getAll);
_.get("/order/:id", getOne);
_.delete("/order/:ids", ordDelete);
