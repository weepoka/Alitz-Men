...
const allStudent = tryCatch(async (req, res, next) => {
  ...
  if (all.length < 1) {
    return next(new NotFoundError("NO Customer "));
  }
  return res.status(200).json({
    message: `Total  Customer: ${all.length ? all.length : 0}`,
    data: all,
  });
});

const searchStd = tryCatch(async (req, res, next) => {
  const regi = req.params.regi;
  console.log(regi);
  const resp = await Student.findById(regi).populate("order");
  console.log(resp);
  if (!resp) {
    return next(new NotFoundError("NO"));
  }
...
const getAdmin = tryCatch(async (req, res, next) => {
  const { email } = req.body;
  const all = await Student.findOne({ email: email, role: "Admin" });
...
const getAdminn = tryCatch(async (req, res, next) => {
  const all = await Student.findOne({ role: "Admin" });
