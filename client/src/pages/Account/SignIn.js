...
const SignIn = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    if (email && password) {
      try {
        const res = await Api.post("/master/api/v1/admin", { email });
        if (res.data.message === "Ami admin") {
          const adm = res.data.data.email;
          const pass = res.data.data.password;
          if (adm === "mmhasan045@gmail.com") {
            dispatch(adminInfo({ email: adm, password: pass }));
            navigate("/admin/dash");
          } else {
            navigate("/");
          }
